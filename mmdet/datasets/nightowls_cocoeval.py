...
class COCOeval:
    ...
    def _prepare(self, id_setup):
        ...
        for gt in gts:
            ...
            gt['height'] = gt['height'] if 'height' in gt else gt['bbox'][3]
    ...
    def accumulate(self, p=None):
        ...
        M = len(p.maxDets)
        ys = -np.ones(
            (T, R, K, M))  # -1 for the precision of absent categories
    ...
    def summarize(self, id_setup, res_file=None):
        ...
        def _summarize(iouThr=None, maxDets=100):
            ...
            heightStr = '[{:0.0f}:{:0.0f}]'.format(p.HtRng[id_setup][0],
                                                   p.HtRng[id_setup][1])
            occlStr = '[{:0.2f}:{:0.2f}]'.format(p.VisRng[id_setup][0],
                                                 p.VisRng[id_setup][1])
            ...
            print(
                iStr.format(titleStr, typeStr, setupStr, iouStr, heightStr,
                            occlStr, mean_s * 100))
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.fppiThrs = np.array([
            0.0100, 0.0178, 0.0316, 0.0562, 0.1000, 0.1778, 0.3162, 0.5623,
            1.0000
        ])
        self.maxDets = [1000]
        self.expFilter = 1.25
