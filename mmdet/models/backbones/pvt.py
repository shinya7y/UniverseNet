...
class PatchEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        is_h_divisible = img_size[0] % patch_size[0] == 0
        is_w_divisible = img_size[1] % patch_size[1] == 0
        assert is_h_divisible and is_w_divisible, \
...
class PyramidVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(num_stages):
            ...
            num_patches = patch_embed.num_patches if i != num_stages - 1 \
                else patch_embed.num_patches + 1
            pos_embed = nn.Parameter(
                torch.zeros(1, num_patches, embed_dims[i]))
            ...
            block = nn.ModuleList([
                Block(
                    ...
                    sr_ratio=sr_ratios[i]) for depth_idx in range(depths[i])
            ])
            depth_cur += depths[i]
    ...
    def _get_pos_embed(self, pos_embed, patch_embed, H, W):
        ...
        if H * W == self.patch_embed1.num_patches:
            ...
            return F.interpolate(
                pos_embed.reshape(1, patch_embed.H, patch_embed.W,
                                  -1).permute(0, 3, 1, 2),
                size=(H, W),
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            ...
            if i == self.num_stages - 1:
                pos_embed = self._get_pos_embed(pos_embed[:, 1:], patch_embed,
                                                H, W)
            else:
