...
def _load_checkpoint(filename, map_location=None):
    ...
    Args:
        filename (str): Accept local filepath, URL, ``torchvision://xxx``,
            ``open-mmlab://xxx``. Please refer to ``docs/en/model_zoo.md`` for
            details.
...
def load_focalnet_checkpoint(model,
                             filename,
                             map_location='cpu',
                             strict=False,
                             logger=None):
    ...
    for table_key in focal_layers_keys:
        ...
        if len(table_pretrained.shape) != 4:
            ...
            # NOTE: different from interpolation used in self-attention,
            # we use padding or clipping for focal conv
            if fsize1 < fsize2:
        ...
        if table_pretrained.shape != table_current.shape:
            if len(table_pretrained.shape) == 2:
                ...
                if L1 < L2:
                    ...
                    table_pretrained_resized[:2 * dim] = table_pretrained[:2 *
                                                                          dim]
